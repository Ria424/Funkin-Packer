...
class PackProperties extends React.Component<Props, State> {
	...
	applyOptionsDefaults = (data: PackOptions) => {
		...
		data.removeFileExtension = data.removeFileExtension ?? true;
		data.prependFolderName = data.prependFolderName ?? true;
		...
		data.width = data.width ?? 8192;
		data.height = data.height ?? 8192;
		data.fixedSize = data.fixedSize ?? false;
		data.powerOfTwo = data.powerOfTwo ?? false;
		data.spritePadding = data.spritePadding ?? 3;
		data.borderPadding = data.borderPadding ?? 1;
		data.allowRotation = data.allowRotation ?? false;
		data.allowTrim = data.allowTrim ?? true;
		data.trimMode = data.trimMode ?? "trim";
		data.alphaThreshold = data.alphaThreshold ?? 0;
		data.detectIdentical = data.detectIdentical ?? true;
		data.sortExportedRows = data.sortExportedRows ?? true;
		data.repackUpdateFileName = data.repackUpdateFileName ?? true;
		data.statsSI = data.statsSI ?? 1024;
		var optimalPacker = getPackerByType("OptimalPacker");
		data.packer = getPackerByType(data.packer).packerName;
